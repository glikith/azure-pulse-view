...
serve(async(req)=>{
  ...
  try{
    ...
    const{interval,ms}=DURATION[timeRange]||DURATION["PT1H"];
    ...
    const j=await r.json();if(!r.ok)throw{error:true,userMessage:j.error?.message||"Failed to fetch metrics"};
